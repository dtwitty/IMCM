...
class Sorter():
	...
	def agg_ratio_sort(self, k):
		agg_ratio = [float(x)/float(y + x) for x, y in zip(self.agg_win, self.agg_loss)]
		best_coaches = self.best_k(agg_ratio, 5)
		self.print_result(best_coaches)

	# Disadvantage
	# Some coach never even make once into playoff and win 5 season games then quit
	def agg_ratio_sort2(self, k):
		agg_ratio = [(float(x) ** 2)/float(y + x) for x, y in zip(self.agg_win, self.agg_loss)]
		best_coaches = self.best_k(agg_ratio, 5)
		self.print_result(best_coaches)
...
print "=================AGG WIN/LOSS RATIO SORT2========================"
sorter.agg_ratio_sort2(5)
