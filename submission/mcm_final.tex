...
\noindent To solve this issue, we think of an algorithm to reduce the originial graph into a DAG:

\begin{algorithm}[H]
\SetLine % For v3.9
\While{not at end of simulation}{
get the pending requests\;
get cabs that are available\;
\While{pending requests $\textbf{and}$ available cabs}{
find the next pending request\;
find cab closest to the request using Djikstra\;
cab handle request\;
once cab handles request, return to a zone proportional to its rate $\lambda$
}
}
\caption{Global version of algorithm- Djikstra's algorithm used}
\end{algorithm}
