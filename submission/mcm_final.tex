...
\\
$h: T \rightarrow \mathbb{I}$ returns the importance multiplier of a game (1 for season game, 2 for play-off games, 4 for championship games), it is also sport specific since different sport have different game structure, both the above structure can be applied to bastketball, football, and baseball.
\\

$\beta$ the score multiple, it is specific to a type of sport. For example, in basketball $\beta$ is set to be larger than in football since the score difference is usually much larger. In our model, it is set to be the medium score difference of a sport in the data.
